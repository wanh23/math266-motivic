...
\begin{proposition}\label{prop:properties-KMW1} \textit{(Properties in $K_1^\MW$)}
\begin{enumerate}
    \item We have that $[1] = 0$.
    ...
    \item $[ab] = [a] +\left\langle a \right\rangle[b]$
...
\begin{enumerate}
    ...
    $[a][a] = [-1][a]$
    \item We have that $[a][b] = \epsilon[b][a]$.
    % \item $[a][a] = [a][-1]$ ??
\end{enumerate}
\end{proposition}
% \begin{proof} $\ $
...
%     Expanding, we get
%     \begin{align*}
%         [a] (1 + \eta[-a])[1-a^{-1}] &= [a][1-a^{-1}] + [a]\eta[-a][1-a^{-1}] \\
%         &= [a][1-a^{-1}] + [a] \left( [-a \cdot (1-a^{-1})] - [-a] - [1-a^{-1}] \right) \\
%         &= [a][1-a^{-1}] + [a] \left( [1-a] - [-a] - [1-a^{-1}] \right) \\
%         &= [a][1-a^{-1}] - [a][-a] - [a][1-a^{-1}] \\
%         &= 
%     \end{align*}
%     The latter term is equal 
    

    
%     By \autoref{KMW:minus-square-bracket}, we have that
%     \begin{align*}
...
%     By \autoref{KMW:bracket-of-fraction}, we have that $[1/a] = [1] - \left\langle a^{-1} \right\rangle[a] = - \left\langle a^{-1} \right\rangle[a]$. Since $\left\langle a^{-1} \right\rangle = \left\langle a \right\rangle$, 

%     This is proven directly analogously to the proof for Milnor $K$-theory.

%     \item We have
%     \begin{align*}
%         [ab][-ab] = 0,
%     \end{align*}
%     so applying MW2 twice we get
%     \begin{align*}
%         0 &=\left( [a] + [b] + \eta[a][b] \right) \left( [-a] + [b] + \eta[-a][b] \right)
%     \end{align*}
%     ...
...
\end{align*}
Multiplying by $\eta$ and applying MW4 gives zero.

Finally we want to check \textbf{(GW2)}, and by multiplicativity we can assume that $b = 1-a$, from which we get
\begin{align*}
