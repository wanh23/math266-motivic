...
\end{tikzcd} \]
\end{example}
Since $\A^1 \xto{\sim} \ast$ is trivial, and since all our colimits are $\infty$-colimits, we can replace the above diagram with the weakly equivalent span $\ast \from \mathbb{G}_m \to \pt$ and compute the same colimit. This implies that the natural map
