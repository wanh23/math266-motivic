...
If $L/k$ is a Galois field extension then $\Spec(L) \to \Spec(k)$ is a $\Gal(L/k)$-torsor in any topology.
\end{example}
\begin{itemize}
...
\begin{example} Let's double back to \autoref{exa:galois-field-extn-torsor} and actually prove that $\Spec L \to \Spec k$ is a principal $\Gal(L/k)$-bundle when $L/k$ is a Galois field extension.
\end{example}
...
\begin{definition} If $\mathscr{C}$ is a locally category with all finite products (binary products and a terminal object) then a \textit{group object} in $\mathscr{C}$ is the data of an object $G\in \mathscr{C}$ together with morphisms
\begin{align*}
    m \colon G \times G &\to G \\
    e \colon 1 \to G \\
    i \colon G \to G
\end{align*}
called multiplication, identity, and inverse, so that the diagrams commute
\[ \begin{tikzcd}
...
\begin{enumerate}
    ...
    There's a small lie here, the actual functor exhibiting the equivalence is not the Moore complex but the \textit{normalized chains complex}, although the normalized chain complex maps to the Moore complex in a natural way, and this is a chain homotopy equivalence \cite[III.2.4]{GoerssJardine}, so they are essentially the same. For more detail see \cite[\S III.2]{GoerssJardine}.}
...
Covers, their overlaps, triple overlaps, etc. are also conveniently encoded in the data of a simplicial object.\footnote{%
Technically it should be an \textit{augmented simplicial object} since we'd also like to remember the data of the maps $U_i \to X$.}

\begin{notation} If $\left\{ U_i \to X \right\}_{i\in I}$ is a cover, we'll denote by $U_{ij}:= U_i \times_X U_j$, and similarly $U_{ijk}:= U_i \times_X U_j \times_X U_k$.
\end{notation}


\begin{example} Let $\mathcal{U} = \left\{ U_i \to X \right\}$ be an open cover of a variety. We define a simplicial object
\[ \begin{tikzcd}[column sep=small]
...
\end{tikzcd} \]
which we call the \textit{\v{C}ech nerve} associated to the cover, and we denote by $N(\mathcal{U})$. By universal property of the pullback, we obtain three projection maps, which become our face maps $d_i \colon N(\mathcal{U})_2 \to N(\mathcal{U})_1$:
\begin{align*}
    d_0 \colon U_{ijk} &\to U_{ij} \\
    d_1 \colon U_{ijk} &\to U_{ik} \\
    d_2 \colon U_{ijk} &\to U_{jk}.
\end{align*}
Higher face maps are defined analogously. For degeneracy maps, remark that there is a natural diagonal map $U_i\to U_{ii}$, inducing the degeneracies $N(\mathcal{U})_0 \to N(\mathcal{U})_1$. The degeneracies at the next level are
\begin{align*}
    s_0 \colon U_{ij} &\to U_{iij} \\
    s_1 \colon U_{ij} &\to U_{ijj},
\end{align*}
and so on for higher degeneracies.
\end{example}
...
\end{align*}
so that $d_0 H_0^n = f_n$, and $d_{n+1}H_n^n = g_n$, and so that they commute with face and degeneracy maps appropriately.\footnote{%
...
\begin{align*}
    \Hom_{\Fun(\DDelta^\op,\Var_k)}(N(\mathcal{U}), B_\bullet G) \to H^1(X,G),
\end{align*}
and this assignment is surjective.
\end{theorem}
\begin{proof} To specify a map, we need to know what happens on each level, so let's consider the diagram:
\[ \begin{tikzcd}
    BG= & \ast\rar & G \lar[shift left]\lar[shift right] & \cdots \\
    N(\mathcal{U})= & \coprod_i U_i\uar\rar & \coprod_{i,j} U_{ij}\lar[shift left]\lar[shift right]\uar & \cdots \\
\end{tikzcd} \]
On 0-simplices, there is no data, and on 1-simplices, we are obtaining an element we'll call $g_{ij} \in G(U_i \times_X U_j)$. What happens on 2-simplices? The degeneracy maps requires the two diagrams to commute:
\[ \begin{tikzcd}
    G\rar["s_0" above] & G \times G\\
    U_{ij}\uar["g_{ij}" left]\rar["s_0" below] & U_{iij}\uar
\end{tikzcd} \quad\quad 
\begin{tikzcd}
    G\rar["s_1" above] & G \times G\\
    U_{ij}\uar["g_{ij}" left]\rar["s_1" below] & U_{iij}\uar
\end{tikzcd}
\]
We've now incorporated all we can learn from degeneracies. But what is the map $U_{ijk}\to G \times G$ when $i,j,k$ are all distinct? To answer that we consider the degeneracies:
\[ \begin{tikzcd}
    G & G \times G\lar["d_0" above]\\
    U_{ij}\uar["g_{ij}" left] & U_{ijk}\uar\lar["d_0" below]
\end{tikzcd} \quad\quad \begin{tikzcd}
    G & G \times G\lar["d_1" above]\\
    U_{ik}\uar["g_{ik}" left] & U_{ijk}\uar\lar["d_1" below]
\end{tikzcd} \quad\quad \begin{tikzcd}
    G & G \times G\lar["d_2" above]\\
    U_{jk}\uar["g_{jk}" left] & U_{ijk}\uar\lar["d_2" below]
\end{tikzcd} \]
Since the inner and outermost degeneracies $d_0$ and $d_2$ are the projection maps $G \times G \to G$, these relations describe the map $U_{ijk} \to G \times G$. So the mathematical content comes from the inner $d_1$ relation, which implies that $g_{ik}$ must be the \textit{product} of $g_{ij}$ and $g_{ik}$. This is \textit{exactly the 1-cocycle condition}.

We claim that the remaining data of the map $N(\mathcal{U})_\bullet \to B_\bullet G$ is completely determined by this relation.\footnote{%
This can very likely be made precise by the idea that $B_\bullet G $ is 1-truncated, I'd like to find a reference that makes this precise so it's clear how the rest of the argument goes. See maybe \cite[I.7.8]{GoerssJardine}%
}
\end{proof}

\begin{theorem} Two 1-cocycles for the cover $\mathcal{U}$ are cohomologous if and only if they are simplicially homotopic!
\end{theorem}
\begin{proof}[Proof sketch] Let $f,g \colon N(\mathcal{U})_\bullet \to B_\bullet G$ be two maps corresponding to 1-cocycles $(f_{ij})$ and $(g_{ij})$, respectively. Recall simplicial homotopy requires us to define maps $H^n_i \colon N(\mathcal{U})_n \to B_{n+1}G$ for $0\le i\le n$ so that $d_0 H_0 = f_n$, $d_{n+1} H_n = g_n$, and various other relations hold. For us, let's see what $H_0$ looks like:
\def\theshiftamount{1}
\[ \begin{tikzcd}
    {\coprod_i U_i}\rar\dar\ar[dr,"H^0_0" above right] & {\coprod_{i,j}U_{ij}} \lar[shift left=\theshiftamount]\lar[shift right=\theshiftamount]\rar[shift left=\theshiftamount]\rar[shift right=\theshiftamount]\dar\ar[dr,"{H^1_0,H^1_1}" above right] & \coprod_{i,j,k} U_{ijk} \lar \lar[shift left=2*\theshiftamount]\lar[shift right=2*\theshiftamount]\dar\ar[dr,"{H_0,H_1,H_2}" above right] & \cdots \\
...
\end{tikzcd} \]
Since $H_i^1$ is mapping into a product, it is determined by its projections, which are post-composing with $d_0$ and $d_2$. From the relations and the simplicial homotopy condition, we get
\begin{align*}
    d_0 H_0^1 &= f_{ij} \\
    d_2 H_0^1 &= H_0^0 d_1 = \alpha_j,
\end{align*}
so $H_0^1$ is the tuple $(f_{ij},\alpha_j) \colon U_{ij} \to G \times G$.\footnote{There's abuse of notation here with $\alpha_j$.} Similarly we compute
\begin{align*}
    d_0 H_1^1 &= H_0^0 d_0 = \alpha_i \\
    d_2 H_1^1 &= g_{ij},
\end{align*}
so $H^1_1 = (\alpha_i,g_{ij}) \colon U_{ij} \to G \times G$. The remaining relation states that
\begin{align*}
    d_1 H_1^1 &= d_1 H_0^1,
\end{align*}
and since $d_1$ is the multiplication $G \times G \to G$, this tells us that $f_{ij}\alpha_j = \alpha_i g_{ij}$. This is the coboundary condition!

Again we claim that the higher data is determined by what is happening in lower degrees (need to make this formal).
\end{proof}
