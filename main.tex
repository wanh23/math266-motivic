...
\end{align*}
is an equivalence, i.e. a 1-categorical limit (equalizer).
\end{example}
...
\begin{definition} We say $\mathcal{F} \in \Pre(\Sm_S)$ is a \textit{Nisnevich sheaf} if $\mathcal{F}(\emptyset)= \ast$ and $\mathcal{F}$ sends every Nisnevich sheaf to a pullback square.
\end{definition}
...
This is some higher categorical analogue of the fact from topos theory that every topos is the category of sheaves of sets on a site.
...
\begin{proposition} \cite[6.4.4.6]{HTT} Colimits in any $\infty$-topos (in particular in $\Shv_\tau(\mathscr{C})$ are \textit{universal}, meaning they commute with pullbacks.
\end{proposition}
