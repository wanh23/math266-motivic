...
\begin{remark} By cofinality (\autoref{exa:even-odd-cofinal}), this is canonically equivalent to the following two colimits:
\begin{align*}
...
\begin{align*}
    \pi_0 L_\mot F = \colim \left( \pi_0 F \to \pi_0 L_\Nis L_{\A^1} F \to \cdots  \right).
\end{align*}
...
\end{tikzcd} \]
By null homotopy of the composite map of the map to the bottom right we get a lift $F \to (L_\mot F)_{>n}$. Since the target is local, the lift factors through $L_\mot F$, giving a section $s$. Hence we have a retract:
\[ \begin{tikzcd}
