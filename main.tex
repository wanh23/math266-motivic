...
\end{center}
Thank you to Merrick Cai, Juan Fernando Valdes, Jake Levinson, Isabel Longbottom, Sof\'ia Marlasca Apararicio, Matthew Niemiro, and Natalie Stewart for valuable changes and suggestions.
...
\end{align*}
%
\begin{proposition} For any $A \in \HI(k)$ and any $E \in \mathcal{E}_k$ we have that $H^1( \mathbb{G}_m \times \Spec(E);A) = 0$  (see \cite[4.10]{Deglise-finite-corr}).
\end{proposition}
...
\begin{theorem} The category $\HI(k)$ admits a closed symmetric monoidal structure, which we denote by $\otimes_\HI$. The unit for the monoidal structure is given by $\KMW_0 = \mathbf{GW}$. In particular every homotopy sheaf is a module over the Grothendieck--Witt sheaf.
\end{theorem}
...
The $\KMW_0$-module structure on any $M_{-1}$ comes from its action on $\KMW_1$:
\begin{align*}
...
Since contraction is exact, the identification above states that $\Hom(\KMW_1,-)$ is exact, implying that $\KMW_1$ is projective as an object in the abelian category $\HI(k)$. This implies it is flat, and it is moreover faithfully flat.

\begin{proposition}\label{prop:K1MW-ff} 
...
\end{proposition}
\begin{proof} We do this in a few steps. Recall we had this element $\epsilon = - \left\langle -1 \right\rangle = -(1 + \eta[-1])$ which measured the failure of symbols $[a][b]$ to commute with one another.

\textbf{Step 1}: We have that $\epsilon[a] = [a]\epsilon$. This is a routine check:
\begin{align*}
    \epsilon[a] &= - \left( 1 + \eta[-1] \right)[a] = -[a] - \eta[-1][a] \\
    &\overset{\text{MW2}}= -[a] + [-1] + [a] - [(-1)a] \\
    &= -[a] + [a] + [-1] + [a(-1)] \\
    &= -[a] + \eta[a][-1] \\
    &= -[a](1 + \eta[-1]) \\
    &= [a]\epsilon.
...
\textbf{Step 2}: We have that $\epsilon^2 = 1$.
\begin{align*}
    \epsilon^2 &= \left( - 1 - \eta[-1] \right)^2 = (1+\eta[-1])^2 \\
    &= 1 + 2\eta[-1] + \eta^2[-1][-1] \\
    &\overset{\text{MW2}}{=} 1 + 2\eta[-1] + \eta \left( [1] - [-1] -[-1] \right) \\
    &\overset{\ref{exer:bracket-1-is-0-in-MW}}{=} 1 + 2\eta[-1] - 2\eta[-1] \\
    &= 1.
\end{align*}

\textbf{Step 3}: We claim the permutation $(1\ 2\ 3)$ going from
\begin{align*}
    \KMW_3 \cong (\KMW_1)^{\otimes3} \to (\KMW_1)^{\otimes 3} \cong \KMW_3
\end{align*}
is equal to the identity, proving $\KMW_1$ is cyclic. Since $\KMW_n$ is generated by symbols $[u_1, \ldots, u_n]$ for $n\ge 1$, we are reduced to checking that $[c,a,b] = [a,b,c]$ for any $a,b,c \in F^\times$. This is now an immediate check by what we've done:
\begin{align*}
...
This is a generalization of every homotopy sheaf being a $\KMW_0 = \mathbf{GW}$-module as we have seen.
