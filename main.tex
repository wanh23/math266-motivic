...
\end{center}
Thank you to Merrick Cai, Juan Fernando Valdes, Jake Levinson, Isabel Longbottom, Sof\'ia Marlasca Apararicio, Matthew Niemiro, and Natalie Stewart for valuable changes and suggestions. Thank you to Elden Elmanto and Matthias Wendt for answering random motivic questions, and thank you to David Chan and Liam Keenan for letting be bother them about technical categorical questions.
...
By \autoref{thm:restriction-to-Fk}, we can define unramified sheaves of abelian groups via their values on fields. Here we provide some examples.

\subsection{Example: Milnor $K$-theory}
...
\begin{enumerate}
    \item $\left\{ a \right\}\cdot \left\{ 1-a \right\} = 0$
...
\subsection{Example: Milnor--Witt $K$-theory} We refer the reader to \cite{Deglise,Carlier} for more in-depth discussions of what's found here.
...
\begin{exercise}\label{exer:bracket-1-is-0-in-MW} 
Show that $[1] = 0$ in $K_\ast^\MW(F)$.
\end{exercise}

\begin{exercise}\label{exer:bracket-squared-MW} Show that $[a^2] = (2 + \eta[-1])[a]$.
\end{exercise}
...
\begin{align*}
    \GW(F) &\to K_0^\MW(F) \\
    \left\langle a \right\rangle &\mapsto 1 + \eta[a].
\end{align*}
\end{proposition}
\begin{proof} We first check this is well-defined. We first check (GW3):
\begin{align*}
...
\end{align*}
Hence (GW3) holds. Since $\left\langle a \right\rangle$ is now multiplicative, checking GW1 reduces to showing $\left\langle b^2 \right\rangle = \left\langle 1 \right\rangle = 1 + \eta[1]$, and we recall $[1]=0$. Hence it suffices to show $\eta[b^2] = 0$. By \autoref{exer:bracket-squared-MW}, we have that
\begin{align*}
    \eta[b^2] &= \eta \left( 2 + \eta[-1] \right)[b] \overset{\text{(MW4)}}{=} 0.
\end{align*}
%
Again using multiplicativity of $\left\langle a \right\rangle$, we can assume $b=1-a$ in (GW2), from which we get
\begin{align*}
...
\begin{notation} We have some special notation for various elements in Milnor--Witt $K$-theory:
\begin{enumerate}
    \item We denote by $h := 1 + \eta[-1] \in K_0^\MW(F)$ the \textit{hyperbolic element}. It is equivalently thought of as the form $\left\langle 1,-1 \right\rangle$ in $\GW(F).$
    \item We denote by $\epsilon := - \left\langle -1 \right\rangle = - (1 + \eta[-1])$.
\end{enumerate}
\end{notation}

\begin{remark} The relation (MW4) in \autoref{def:MW} can be rewritten in either of the two ways:
\begin{enumerate}
    \item[(MW4)] $\eta h = 0$
...
The element $\epsilon$ measures the failure of commutativity in $K_\ast^\MW(F)$ as the following exercise illustrates.

\begin{exercise} We have that
\begin{align*}
    [a][b] = \epsilon[b][a].
\end{align*}
\end{exercise}
...
\subsection{The Milnor conjecture}
...
\begin{proposition} In the sheaf topos, we have that
\begin{align*}
    \Omega K(A,n) \cong K(A,n-1).
\end{align*}
\end{proposition}
\begin{proof} This is a formal consequence of the fact that $\pi_i(\Omega X) = \pi_{i+1}X$.
\end{proof}

If $A$ is strictly $\A^1$-invariant then $K(A,n)$ is $\A^1$-local, however it is not clear that motivic localization commutes with taking loops. To pass this statement to the category of motivic spaces, we need the following lemma.
...
\begin{corollary} If $A$ is strictly $\A^1$-invariant, we have an identification in $\Spc(k)$ of the form
\begin{align*}
    \Omega^{1,0} K(A,n) \simeq K(A,n-1).
\end{align*}
\end{corollary}
\begin{proof} The loops comes from the fiber sequence
\begin{align*}
    \Omega K(A,n) \to \ast \to K(A,n).
\end{align*}
The latter two spaces are $\A^1$-local, hence the loop space is as well.
\end{proof}
...
Since colimits in $\Spc(k)$ are pullback-stable, any unpointed motivic space $B$ gives rise to a cofiber sequence of spaces\footnote{%
Here one can compute that $C \smashprod B_+$ is the ``fiberwise'' cover in $\Spc(k)_{/X}$.
}
\begin{align*}
...
\begin{align*}
    \mathbb{G}_m \times B \to \A^1 \times B \to \P^1 \smashprod B_+.
\end{align*}
\end{example}
...
\begin{definition} If $F$ is a sheaf of pointed sets, we define the \textit{contraction} of $F$ to be the sheafification of the presheaf $F_{-1}$ definer as the kernel
\begin{align*}
    0 \to F_{-1}(U) \to F(U \times \mathbb{G}_m) \to F(U) \to 0,
\end{align*}
where the latter map is $\id \times 1 \colon U \to U \times \mathbb{G}_m$. See \cite[2.1.10]{Freudenthal}.
\end{definition}
...
\begin{remark} Now suppose If $G$ is strongly $\A^1$-invariant, we have our cofiber sequence
\begin{align*}
    \mathbb{G}_m \times U \to \A^1 \times U \to \P^1 \smashprod U_+.
\end{align*}
...
\begin{proof} The map
\begin{align*}
    H^1(X,G) = H^1(\A^1 \times X,G) \to H^1(\mathbb{G}_m \times X,G)
\end{align*}
...
\begin{align*}
    G_{-1}(-) \cong H^1(\P^1 \smashprod (-)_+, G) = \pi_0 \Map( \Sigma^{2,1} (-)_+, BG) = \pi_0 \Map ( X_+, \Omega^{2,1} BG).
\end{align*}
\end{proof}
...
\begin{enumerate}
    ...
    \end{align*}
    In particular if $A$ is strictly $\A^1$-invariant so is $A_{-1}$.
\end{enumerate}
\end{corollary}
\begin{proof} $\ $
\begin{enumerate}
...
\end{align*}
%
\begin{proposition} For any $A \in \HI(k)$ and any $E \in \mathcal{E}_k$ we have that $H^1( \mathbb{G}_m \times \Spec(E);F) = 0$  (see \cite[4.10]{Deglise-finite-corr}).
\end{proposition}
...
\begin{align*}
    A \to B \to C
\end{align*}
is an exact sequence in $\HI(k)$. Since $H^1(\mathbb{G}_m \times -, A) = 0$, the sequence
\begin{align*}
