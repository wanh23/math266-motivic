...
\end{theorem}
\begin{proof}[Proof intuition] Let $M \in \HI(k)$. Then any map of pointed sheaves of sets $\Sigma (\mathbb{G}_m^{\smashprod n}) \to M$ can be viewed as a map of pointed motivic spaces, and we see by adjunction:
\begin{align*}
...
\begin{align*}
    \pi_0 L_\mot F = \colim \left( \pi_0 F \to \pi_0 L_\Nis L_{\A^1} L_\Nis F \to \cdots  \right).
\end{align*}
Since the colimit will be a quotient of the sheaf $\pi_0 F$, which is trivial, the result follows
\end{proof}
...
\begin{proof}[Proof of \autoref{thm:unstable-connectivity}] The base case is $n=0$, which we already proved (\autoref{cor:unstable-connectivity-0}), so now we can induct. Consider the fiber sequence in the sheaf topos
\begin{align*}
...
\[ \begin{tikzcd}
     & X\dar\ar[dr]\ar[dl,dashed]\\
    (L_\mot F)_{>n}\rar & L_\mot F \rar & (L_\mot F)_{\le n}\\
\end{tikzcd} \]
By null homotopy of the composite map of the map to the bottom right we get a lift $X \to (L_\mot F)_{>n}$. Since the target is local, the lift factors through $L_\mot F$, giving a section $s$. Hence we have a retract:
\[ \begin{tikzcd}
...
\end{align*}
is the initial map to a strongly invariant sheaf of groups.
\end{proposition}
