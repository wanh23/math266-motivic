...
\begin{enumerate}
    ...
    \item The map
    \begin{align*}
        \mathcal{F}(X) \bigcap_{x\in X^{(1)}} \mathcal{F}(\Spec \O_{X,x})
    \end{align*}
    is an injection.
\end{enumerate}
\end{definition}
...
Unramified sheaves are determined in a precise sense by their values on fields.

\subsection{Unramified sheaves from theories on fields}


\begin{definition} A \textit{homotopy sheaf} is a strongly (=strictly) $\A^1$-invariant sheaf of abelian groups. We denote by $\HI(k) \subseteq \Ab_\Nis(k)$ the full category of homotopy sheaves.
\end{definition}
...
\begin{notation} Let $\mathcal{F}_k$ denote the category of fields $F/k$ so that the transcendence degree of $F/k$ is finite.
\end{notation}
...
\subsection{Example: Milnor--Witt $K$-theory}

\begin{definition} \textit{Milnor--Witt $K$-theory} of a field $F$ is defined to be the graded algebra $K_\ast^\MW(F)$ defined by symbols $[a]\in K_1^{\MW}(F)$ for $a\in F^\times$ and $\eta \in K_{-1}^\MW(F)$, modulo the relations:
\begin{enumerate}
    \item $[a][1-a] = 0$ for $a\ne 0,1$
    \item $[a][b] = [a]+[b] + \eta[a][b]$
    \item $\eta[a]=[a]\eta$
    \item $\eta(2+\eta[-1]) = 0$.
\end{enumerate}
\end{definition}
...
\begin{enumerate}
    \item $\left\langle a \right\rangle \left\langle b \right\rangle = \left\langle ab \right\rangle$ 
    \item $\left\langle a \right\rangle + \left\langle b \right\rangle = \left\langle ab(a+b) \right\rangle + \left\langle a+b \right\rangle$
    \item $\left\langle 1 \right\rangle + \left\langle -1 \right\rangle = \left\langle a \right\rangle + \left\langle -a \right\rangle$
    \item $\left\langle ab^2\right\rangle=\left\langle a\right\rangle$.
\end{enumerate}
\end{proposition}
...
\begin{notation} We denote by $h := 1 + \eta[-1] \in K_0^\MW(F)$ which is the same as the form $\left\langle 1,-1 \right\rangle \in \GW(F)$.
\end{notation}

\begin{definition} We define the \textit{Witt ring} to be the quotient $W(F) := \GW(F)/h$.
\end{definition}

\begin{definition} We define the \textit{fundamental ideal} $I(F)$ to be the kernel of the rank homomorphism $\GW(F) \to \Z$. We denote by $I^j$ the powers of the fundamental ideal.
\end{definition}

\begin{example} \textit{(Examples of fundamental ideals)}
\begin{enumerate}
    \item We have that $I(F)$ is generated by $\left\langle a \right\rangle - \left\langle 1 \right\rangle$ for $a\in F^\times$. In particular it vanishes if $F$ is quadratically closed.
    \item $I(\R) \cong \Z$, generated by $\left\langle 1  \right\rangle - \left\langle -1 \right\rangle$. We can check that
    \begin{align*}
        (\left\langle 1 \right\rangle - \left\langle -1 \right\rangle)^n = n \left\langle 1 \right\rangle - n \left\langle -1 \right\rangle,
    \end{align*}
    so $I(\R) \cong I^2(\R) \cong \cdots \cong I^n(\R)$.

    \item Note that $I^2(\R) \subseteq I(\R)$ is isomorphic to $2\Z \subseteq \Z$. Hence
    \begin{align*}
        I(\R)/I^2(\R) \cong \Z/2.
    \end{align*}
    This is a more general fact for any field that
    \begin{align*}
...
\begin{proposition} There is a pullback square of abelian groups for any field $F$
\[ \begin{tikzcd}
...
\begin{proposition} Given two quadratic forms $\alpha,\beta \in \GW(F)$, they agree if and only if they agree in $I^n(F)$ for every $n$, and therefore if and only if they agree in the associated graded $I^n(F)/I^{n+1}(F)$ for every $n$.
\end{proposition}
...
\begin{aligned}
    K_n^M(F)/2 &\to I^n/I^{n+1} \\
    \left\{ a_1, \ldots, a_n \right\} &\mapsto \prod_{i=1}^n \left( \left\langle a_i \right\rangle - \left\langle 1 \right\rangle \right).
\end{aligned}
\end{equation}
\end{proposition}
...
% \end{align*}
% is an equivalence of spaces. Each space is 1-truncated and we have an isomorphism on $\pi_0$ since $H^0(-,G)$ is $\A^1$ invariant. So we just want to show an isomorphism on $\pi_1$ for each choice of basepoint.
% Pick some $x \in \pi_0(B_\Nis G)(X)$, and consider the induced map..... \todo finish from Tom's note
\end{proof}
...
\end{align*}
\end{proposition}
\begin{proof} This is a formal consequence of $\pi_i(\Omega X) = \pi_{i+1}X$.
\end{proof}
