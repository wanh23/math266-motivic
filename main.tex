...
\section{\sectionheader Motivic spaces}

Recall we have
\begin{align*}
    \Shv_\Nis(\Sm_S) \subseteq \Pre(\Sm_S),
\end{align*}
the accessible subcategory of Nisnevich sheaves.
...
\end{proposition}
\begin{proof} It suffices to observe that the presheaves are identical, which is true because for any $U\in \Sm_S$, we have that $h_S(U) = \Hom_S(U,S)$, which is just a point consisting of the structure map (or since $S$ is terminal in $\Sm_S$).
\end{proof}
...
A \textit{vector bundle torsor} is an affine map $Y \to X$ which is a torsor for an algebraic vector bundle $E \to X$. Explicitly, $Y$ is locally isomorphic to $E$, but the patching maps are algebraic and need not be affine.
\end{definition}
...
\begin{proposition} Both of $\Pre_{\A^1}(\Sm_S)$ and $\Pre_{\htp}(\Sm_S)$ are accessible subcategories of $\Pre(\Sm_S)$.
\end{proposition}
...
\end{proposition}
\begin{proof} We want to prove for any $X\in \Sch_S$ that the projection map $\pi \colon X \times \A^1 \to X$ induces an equivalence
\begin{align*}
...
\end{align*}
In other words we could equivalently define motivic spaces via $L_{\A^1}$, or via a different localization $L_\htp \colon \Pre(\Sm_S) \to \Pre_\htp(\Sm_S)$. The former is more common, although the latter has some nice advantages (see \cite[p.~204]{Hoyois6} for a discussion, and \cite[3.13]{Hoyois6} for the equality above).
\end{remark}
...
\begin{align*}
    L_{\A^1} h_Y = L_{\A^1}(U_0 \times\A^n) \amalg_{L_{\A^1}(U_{01})} L_{\A^1}(U_1 \times \A^n) \simeq .
\end{align*}
...
\begin{proposition} We have that $L_\mot$ is \textit{not left exact}. In particular $\Spc(k)$ is not an $\infty$-topos.
\end{proposition}
\begin{proof} We will come back to prove this later when we have a little more machinery.
\end{proof}
...
We have that $\Delta^0 \cong S$ in $\Spc(S)$, and moreover these are terminal, hence we denote them by $\ast$.
\end{proposition}
...
\begin{theorem} (Jouanalou--Thomason trick) Suppose that $S$ is qcqs and further suppose either $S$ is affine or it is noetherian, regular, and separated. Then for any quasi-projective $X\in \Sch_S$, there exists an affine bundle $Y \to X$, where $Y$ is affine.
\end{theorem}
