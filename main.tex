...
This has the property that it represents twisted cohomology
\begin{align*}
    \left[ X, K^G(M,n) \right] = H^n(X,M(G)).
\end{align*}
%

\begin{proposition} (see \cite[6.1]{AF3fold}) If $X$ is a pointed connected motivic space, then $f_n \colon \tau_{\le n+1} X \to \tau_{\le n} X$ is a twisted $\A^1$-principal fibration, classified by a map
\[\begin{tikzcd}
...
\begin{remark} (Historical background) More generally given a rank $r$ algebraic vector bundle over a smooth $d$-dimensional affine variety, if $r>d$ then it splits off a trivial summand by Serre splitting and Bass cancellation. So $r=d$ is the first interesting case. For $r=d=2$, this was proven by Murthy and Swan \cite{MurthySwan}. Murthy and Mohan Kumar showed this for $r=d=3$ in \cite{KumarMurthy}, and finally Murthy proved the general result \cite{Murthy94}.
\end{remark}
